...
;--------------------------------------------------------------------
;[PIN NUMBERS]
BasePin 		con P3	;Base Connection
ShoulderPin 	con P4	;Shoulder Connection
ElbowPin  		con P5	;Elbow Connection
WristPin		con P6	;Wrist Connection
GripperPin		con P7	;Gripper Connection
WristRotatePin	con P8	;Wrist Rotation Connection (Optional)

RightChannelPin	con P1	;Sabertooth Motor driver Right channel
LeftChannelPin  con P0	;Sabertooth Motor driver Left channel
...
;[STORED ANGLES]
BaseAnglestore 			var sword	;Stored Angle of the Base servo
ShoulderAnglestore 		var sword	;Stored Angle of the Shoulder servo
ElbowAnglestore  		var sword	;Stored Angle of the Elbow servo
WristAnglestore			var sword	;Stored Angle of the Wrist servo
Gripperstore			var sword	;Stored Angle of the Gripper servo
WristRotateAnglestore	var sword	;Stored Angle of the Wrist Rotation servo
;--------------------------------------------------------------------
;[POSITIONS]
...
;[STORED POSITIONS]
WristPosXstore			var slong	;Position of the Gripper used for input
WristPosYstore			var slong
GlobalGripperAnglestore	var sword	;Angle of the Gripper used for input
;--------------------------------------------------------------------
;[Gripper]
...
TargetWristAnglestore	var sword
MaxStepWristAngle	con 40
TargetGripper1		var sword
TargetGripperstore		var sword
...
Armreturn var bit			;toggles arm return
;====================================================================
;Interrupt init 
...
;[ROVER CALCULATIONS]
  ...
  IF RoverOn THEN ;AND ArmOn=FALSE THEN
    IF Prev_RoverOn=0 THEN
      Sound P9,[60\2500,80\3000,100\3500,120\4000]
    ENDIF
...
Ps2Input:
  ...
  IF ControlMode = ARM_MODE THEN
    ...
    IF ArmOn THEN
	  ...
	  IF (DualShock(1).bit4 = 0) and LastButton(0).bit4 THEN	;D-Up Button test
	  	Baseanglestore = Baseangle1
	  	globalgripperanglestore = globalgripperangle1
	  	WristPosXstore = WristPosX
	  	WristPosYstore = WristPosY
	  	TargetWristAnglestore = TargetWristAngle1
	  	TargetGripperstore = TargetGripper1
	  	sound P9, [100\4000,100\5000]
	  	
	  ENDIF
		
	  IF (DualShock(1).bit6 = 0) and LastButton(0).bit6 THEN	;D-Down Button test
	  	
	    globalgripperangle1 = globalgripperanglestore
	  	WristPosX = WristPosXstore
	  	WristPosY = WristPosYstore
	  	TargetWristAngle1 = TargetWristAnglestore
	  	TargetGripper1 = TargetGripperstore
	  	sound P9, [100\5000,100\4000]
	  ...
	  IF (DualShock(1).bit7 = 0) and LastButton(0).bit7 THEN	;D-Left Button test
	  	IF armreturn = 0 then
	  		armreturn = 1
	  		sound P9, [60\2500,60\3500]
	  	else
	  		armreturn =0
	  		sound P9, [60\1500,100\1000]
	  	endif
	  ENDIF
